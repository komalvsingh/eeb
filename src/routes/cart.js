...
const router = Router();
import  isAuth from '../utils/isAuth.js'; // Update the import path as needed
import logger from '../utils/logger.js';
import Cart from '../models/cart.js'; // Assuming 'findOne' and 'findOneAndUpdate' are part of Cart
...
router.post('/', isAuth, async (req, res) => {
  try {
    ...
    const updatedCart = await Cart.findOneAndUpdate(filter, update, options).populate('products.product');
    const total = updatedCart.products.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
    return res.json({
...
router.delete('/:id', isAuth, async (req, res) => {
  try {
    ...
    const updatedCart = await Cart.findOne({ user: req.user._id })
      ...
      .select('-user')
      .lean();
    const cartTotal = updatedCart.products.reduce((total, item) => total + item.quantity * item.product.price, 0);
